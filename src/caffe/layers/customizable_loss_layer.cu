...
void CustomizableLossLayer<Dtype>::Forward_gpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top)
{
	if (this->alter_forward_gpu_ != 0)
	{
		this->alter_forward_gpu_(bottom, top);
	}
...
template <typename Dtype>
void CustomizableLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
    const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom)
{
	if (this->alter_backward_gpu_ != 0)
	{
		this->alter_backward_gpu_(bottom, propagate_down, top);
	}
#if 0
  for (int i = 0; i < 2; ++i) {
    if (propagate_down[i]) {
...
INSTANTIATE_LAYER_GPU_FUNCS(CustomizableLossLayer);
